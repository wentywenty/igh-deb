---
name: Bug / Issue
about: 报告问题或提出需求 / Report a bug or request a feature
labels: triage
---

<!-- 请勾选类型 / Check the issue type -->
- [ ] 🔨 构建问题 Build issue（deb 编译失败、版本不一致 / compile fail, version mismatch）
- [ ] 🔧 DKMS 问题 DKMS issue（内核模块编译失败 / kernel module build fail）
- [ ] 🌐 驱动问题 Driver issue（igb/generic/r8169 等不工作 / driver not working）
- [ ] 🛠️ 新功能 / 新驱动请求 Feature request / new driver request

## 环境 / Environment

- 架构 Arch: [amd64 / arm64]
- 系统 OS: [RobopiOS / Ubuntu 22.04 / Ubuntu 24.04]
- 内核版本 Kernel: <!-- uname -r 输出，如 6.1.99-rt36-rockchip-rk3588 -->
- 包版本 Version: <!-- dpkg -l ethercat-igh 输出 -->

## 现象 / What happened

<!-- 发生了什么？截图/日志/命令输出 -->
<!-- What went wrong? Screenshots, logs, command output -->

## 复现步骤 / Steps to reproduce

1.
2.
3.

## 期望行为 / Expected behavior

<!-- 你期望的正确结果 / What should happen instead -->

> ⚠️ **禁止使用 AI 生成 Issues。AI 生成的 Issue 将被直接关闭。**
> **Do not use AI to generate Issues. AI-generated issues will be closed immediately.**
